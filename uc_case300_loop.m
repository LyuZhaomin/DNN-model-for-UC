...
savecase('case300_custom', mpc);  
mpc_test = loadcase('case300_custom');
mpc_test = ext2int(mpc_test); %buses must be numbered consecutively in bus 
                              % matrix; use ext2int() to convert to
                              % internal ordering
...
num_samples = 1;  % Total data points
...
while sample_count < num_samples
    % 2.1. Generate new demand profile
    ...
    % 2.2. COMPLETELY reset constraints
    UC_constraints = {};
    
    % 2.3. REBUILD ALL CONSTRAINTS (in this order)
    ...
    % 2.4. Solve fresh problem
    diagnostics = optimize(UC_constraints, UC_obj, options);
    
    % 2.5. Store if feasible
    if diagnostics.problem == 0
        sample_count = sample_count + 1;
        inputs(:,:,sample_count) = P_d;
        outputs(:,:,sample_count) = value(P);
        % P_value = value(P);
        % gens_to_plot = [1, 16, 41, 42];  % Indices of generators to visualize
        % P_selected = P_value(gens_to_plot, :);  % Rows = selected gens, cols = hours
        % figure;
        % plot(P_selected', 'LineWidth', 1.5);  % Transpose to [T × N_selected]
        % title('Case 300 Generation Dispatch Over Time');
        % xlabel('Hour');
        % ylabel('Power Output (MW)');
        % legend_labels = arrayfun(@(x) sprintf('Gen %d', x), gens_to_plot, 'UniformOutput', false);
...
% Step 3: Save Dataset
save('UC_dataset_case300.mat', 'inputs', 'outputs');
...
save('UC_train_test_case300.mat', 'X_train', 'Y_train', 'X_test', 'Y_test');